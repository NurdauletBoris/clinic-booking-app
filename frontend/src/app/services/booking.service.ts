...
export class BookingService {
  ...
  private doctors: Doctor[] = [
    ...
    { id: 10, name: 'Нурмаганов Серик Балташевич',       specialty: 'Челюстно-лицевой хирург',       experience: 35, rating: 4.71, price: 26000, clinic: 'Medical Park',                        address: 'ул. Розыбакиева, 105Б',               photoUrl: 'https://idoctor.kz/images/doctors/15001/14569/T0qtawrmblXj1MUAv8aZAjpJK6Y5lQjUD7HtfBaC_200x200.png',  slots: ['15:00', '16:00', '17:00', '18:00', '19:00'] },
  ...
  private seedReviews: Review[] = [
    { doctorName: 'Бишманов Рустем Какимжанович', specialty: 'Уролог',    comment: 'Очень внимательный врач, процедура прошла безболезненно и быстро. Рекомендую!',              photo: 'https://idoctor.kz/images/doctors/1229001/1228832/MvW73Y6S9R9CAnfXv23rK38V_180x180.png', rating: 5 },
    { doctorName: 'Кадирова Зайтунам Турсуновна',  specialty: 'Терапевт', comment: 'Профессионал своего дела. Поставила диагноз, который не могли определить в других клиниках.', photo: 'https://idoctor.kz/images/doctors/1229001/1228490/KshYVpXm9F9CAnfXv23rK38V4vW73Y6S_180x180.png', rating: 5 },
    { doctorName: 'Сулейменов Ерлан Талгатович',   specialty: 'Хирург',   comment: 'Спасибо доктору за чуткость и подробную консультацию. Теперь только к вам!',                photo: 'https://idoctor.kz/images/doctors/1229001/1228741/JpXv3S2m9R9CAnfXv23rK38V4vW73Y6S_180x180.png', rating: 5 },
  ];

  private appointmentsSubject = new BehaviorSubject<Appointment[]>([]);
  appointments$ = this.appointmentsSubject.asObservable();

  private reviewsSubject = new BehaviorSubject<Review[]>(this.seedReviews);
  ...
  private calcRating(doctorName: string, allReviews: Review[]): number {
    ...
    return Math.round((userSum / userReviews.length) * 100) / 100;
  }

  getDoctors(): Observable<Doctor[]> { return this.doctors$; }
  ...
  addReview(newReview: Review) {
    this.reviewsSubject.next([...this.reviewsSubject.value, newReview]);
  }

  private mergeApiReviews(apiReviews: any[]): void {
    if (!apiReviews || apiReviews.length === 0) return;

    const mapped: Review[] = apiReviews.map(r => ({
      doctorName:   r.doctor_name_out ?? r.doctorName  ?? '',
      specialty:    r.specialty_out   ?? r.specialty   ?? '',
      photo:        r.photo_out       ?? r.photo       ?? '',
      comment:      r.comment         ?? '',
      rating:       r.rating,
      date:         r.date            ?? '',
      reviewerName: r.reviewerName    ?? r.reviewer_name ?? 'Аноним',
    }));

    const existing    = this.reviewsSubject.value;
    const existingKeys = new Set(existing.map(r => `${r.doctorName}||${r.comment}`));
    const newOnes     = mapped.filter(r => !existingKeys.has(`${r.doctorName}||${r.comment}`));

    if (newOnes.length > 0) {
      this.reviewsSubject.next([...existing, ...newOnes]);
    }
  }

  fetchReviewsFromApi(doctorName: string): void {
    this.http.get<any[]>(`/api/reviews?doctor=${encodeURIComponent(doctorName)}`).pipe(
      catchError(() => of([]))
    ).subscribe(data => this.mergeApiReviews(data));
  }

  fetchAllReviewsFromApi(): void {
    this.http.get<any[]>('/api/reviews').pipe(
      catchError(() => of([]))
    ).subscribe(data => this.mergeApiReviews(data));
  }
  ...
  bookAppointmentViaApi(appointment: Appointment): Observable<{ success: boolean; error?: string }> {
    return this.http.post<Appointment>('/api/appointments', appointment).pipe(
      map(() => { this.bookAppointment(appointment); return { success: true }; }),
      catchError(err => {
  ...
  addReviewViaApi(review: Review): Observable<{ success: boolean; error?: string }> {
    return this.http.post<Review>('/api/reviews', review).pipe(
      map(() => { this.addReview(review); return { success: true }; }),
      catchError(err => {
